...
type props<T> = {
  displayComponent: displayType<T>;
  isLoading: boolean;
  changeHandler: (data: string) => any;
  data: T[];
  error?: string;
  textFieldProps?: Omit<TextFieldProps, "variant">;
  initValue?: any; // INITIALIZE WITH A VALUE
};

const AppSearchField = <T,>({
  changeHandler,
  data,
  displayComponent,
  error,
  ...
  const [selected, setSelected] = useState<T | null>(initValue || null);
  const [focused, setFocused] = useState(false);
  const bodyRef = useHideOnClickedOutside(setFocused.bind(undefined, false));
  ...
  return (
    <div ref={bodyRef} className="relative w-full">
      ...
      {!selected && focused && (
        <div
          className="absolute top-full z-30 w-full rounded-md bg-white py-2"
          style={{
      ...
      {selected && (
        <>
          {displayComponent(selected, "selected", () => {
            setSelected(null);
          })}
        </>
      )}
